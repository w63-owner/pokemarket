...
import { Avatar, Tabs, TabsContent, TabsList, TabsTrigger, Text } from "@/components/ui";
...
function AboutTab({ profile }: { profile: Profile }) {
  ...
  const muted = useThemeColor("mutedForeground");
  const flag = profile.country_code ? countryCodeToFlag(profile.country_code) : "";
  ...
  return (
    <View className="gap-5 pt-4">
      ...
      {profile.bio ? (
        <Text className="self-center max-w-md text-center text-sm leading-5">
          {profile.bio}
        </Text>
      ) : null}
