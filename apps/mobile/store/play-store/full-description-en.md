DeckDealr — The marketplace built 100% for Pokémon cards

Buy and sell your favorite Pokémon cards stress-free, from your phone. A French C2C platform dedicated to collectors, players and investors of the franchise.

✨ WHY DECKDEALR?

🤖 30-second AI scan
Point your camera at your card, AI auto-detects the name, set, number and rarity. No more typing everything by hand.

💳 Instant Google Pay checkout
Pay with one tap using your fingerprint. No card numbers to type, no passwords to remember.

💰 Low fees — only 4%
vs. 5–10% on generic marketplaces. You keep more.

🔒 Secure payments with delayed seller transfers
Sellers are credited only after the buyer confirms receipt.

🏆 Certified graded cards
Auto-verification of PSA, BGS, CGC and ARS cards with certificate number display.

🛒 FOR BUYERS

• Full catalog: Wizards era (Base Set, Jungle, Fossil...) to the latest expansions (Scarlet & Violet, 151, Prismatic Evolutions)
• Advanced filters: set, condition, price, language, grading
• Push notifications on saved searches
• Favorites system
• 30-day price history
• Built-in messaging
• Seller rating system

📦 FOR SELLERS

• AI-assisted listing: 2 photos are enough
• Bank transfer payouts, no hidden fees
• Clear sales dashboard
• Built-in shipment tracking
• Buyer rating

🛡️ SAFETY AND TRANSPARENCY

• Mandatory KYC for all sellers (PSD2 compliant)
• Dispute system with 48h human mediation
• Front + back photos required for every listing
• EU-hosted, GDPR compliant
• Fingerprint or Face Unlock at login

🎮 FRENCH COMMUNITY

Join the largest French community dedicated to Pokémon cards. Chat, negotiate, share your pulls and find the rare gem.

📜 LEGAL

DeckDealr is an independent C2C platform. We are not affiliated with The Pokémon Company, Nintendo, Game Freak or Creatures. Pokémon and Pokémon character names are trademarks of Nintendo.

Download DeckDealr now and bring your collection back to life!

— The DeckDealr team
