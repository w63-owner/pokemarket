  ...
  const client = {
    ...
    async rpc(name: string, params: Record<string, any>) {
      ...
      if (name === "release_escrow_funds") {
        ...
        // never would in prod. total_amount - fee_amount is the exact amount
        // credited to pending_balance at payment finalization.
        const sellerNet =
          Math.round(
      ...
      if (name === "add_wallet_available_balance") {
        const { p_user_id, p_delta } = params;
        ...
        if (!(p_delta > 0)) {
          return {
            data: null,
            error: { code: "P0001", message: "Invalid amount" },
          };
        }

        wallet.available_balance =
          Math.round((Number(wallet.available_balance ?? 0) + p_delta) * 100) /
          100;
