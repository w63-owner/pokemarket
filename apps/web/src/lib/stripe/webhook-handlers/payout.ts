 ...
 *
 * Do NOT restore the platform wallet here. A `payout.failed` event means the
 * connected-account bank payout failed after our platform transfer had already
 * succeeded, so the money is still on the seller's connected Stripe balance.
 * Re-crediting `wallets.available_balance` would let the seller request a
