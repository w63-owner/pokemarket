 ...
 *   1. Mark the payout record as failed for audit/history.
 *   2. Notify the seller with a clear next-step ("update your IBAN").
 *
 * Identification:
 *   The payout route stores `metadata.user_id` on explicit connected-account
 *   payouts. If the metadata is missing (e.g. legacy payout or Stripe-created
 *   automatic payout), we fall back to looking up via stripe_account_id.
 *
 * Do NOT restore the in-app wallet here. By the time a connected-account
 * `payout.failed` event exists, the platform transfer has already succeeded
 * and the funds are back on the connected account's Stripe balance. Restoring
 * PokeMarket's wallet too would let the seller request a second platform
 * transfer for the same earnings.
 */
export async function handlePayoutFailed(
  ...
  Sentry.captureMessage(
    `Payout failed after Connect transfer: ${payout.id} amount=${amountEur}€ user=${sellerId} reason=${payout.failure_message ?? payout.failure_code}`,
