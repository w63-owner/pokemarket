 ...
 *   1. Mark the payout record failed for seller-visible history.
 *   2. Notify the seller with a clear next-step ("update your IBAN").
 *
 * Important accounting note:
 *   Our payout route first transfers money from the platform to the connected
 *   account, then creates a connected-account bank payout. A later
 *   `payout.failed` means the bank payout failed after the platform transfer
 *   succeeded; the funds remain on the connected account. Re-crediting the
 *   platform wallet here would let the seller withdraw the same balance again.
 *
 * Identification:
