 ...
 *
 * Identification:
 *   The payout route stores `metadata.user_id` on every connected-account
 *   payout it creates. We read it back here to find the seller. If the metadata
 *   is missing (e.g. legacy payout from an admin), we fall back to looking up
 *   via stripe_account_id.
 *
 * IMPORTANT: Do NOT restore the app wallet here. This event means the bank
 * payout from the connected Stripe account failed after the platform transfer
 * already succeeded; Stripe returns the funds to the connected account balance,
 * not to the platform. Re-crediting our wallet ledger would let the seller
