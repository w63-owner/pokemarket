 ...
 *   1. Mark payout history as failed.
 *   2. Notify the seller with a clear next-step ("update your IBAN").
 *
 * Do NOT restore `wallets.available_balance` here. PokeMarket uses separate
 * charges and transfers: by the time a connected-account bank payout fails,
 * the platform -> connected-account transfer has already succeeded and the
 * funds are on the seller's Stripe balance. Re-crediting the app wallet would
 * let the seller withdraw the same earnings again.
 *
 * Identification:
