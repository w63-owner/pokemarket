 ...
 *
 * CRITICAL: the Stripe object must bind to this transaction via metadata and
 * must match the persisted total (EUR cents). The success page accepts a
 * query `session_id`, so without these checks a buyer can reuse any paid
 * session to finalize a different (more expensive) PENDING_PAYMENT order.
 *
 ...
 *
 * Same binding rules as the Checkout Session path: metadata + exact EUR
 * amount must match the target transaction.
 */
