...
export async function POST(request: Request) {
  ...
  try {
    switch (event.type) {
      ...
      // The mobile app pays via a direct PaymentIntent (PaymentSheet) rather
      // than a hosted Checkout Session. Both terminate the transaction the
      // same way: tag it PAID, run side-effects, mark the listing SOLD.
      ...
      case "payment_intent.canceled":
        await handlePaymentIntentFailed(
          event.data.object as Stripe.PaymentIntent,
          admin,
          event.type === "payment_intent.canceled" ? "CANCELLED" : "CANCELLED",
        );
        break;
...
async function handleCheckoutFailed(
  ...
  await applyFailureToTransaction(admin, transactionId, listingId, targetStatus);
}
...
async function handlePaymentIntentSucceeded(intent: Stripe.PaymentIntent) {
  ...
  if (!transactionId || !listingId) {
    // Not one of ours (e.g. Connect-driven PaymentIntent without metadata).
    // Silently ignore so unrelated PIs don't trigger 500s on our webhook
    // endpoint (Stripe will retry indefinitely otherwise).
    return;
  }
...
/**
 * Shared rollback used by both the Checkout-Session-failed and
 * PaymentIntent-failed/canceled webhooks. Releases the LOCKED listing back
 * to ACTIVE (or RESERVED if there's still an accepted offer).
 */
async function applyFailureToTransaction(
  admin: AdminClient,
