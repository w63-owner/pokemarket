...
describe("webhooks/stripe — payout.failed accounting", () => {
  it("connected-account payout failure does not restore the app wallet after platform transfer succeeded", async () => {
    stripeConstructEventImpl = () => ({
      id: "evt_payout_failed_no_wallet_restore",
      type: "payout.failed",
    ...
    const scenario = basicScenario();
    scenario.profiles![0].stripe_account_id = "acct_seller_1";
    ...
    const db = createMockDb(scenario as any);
    mockClient = db.client;
