...
// Mobile callback target. Pairs with `WebBrowser.openAuthSessionAsync` on
// the RN side, which dismisses the in-app browser as soon as Stripe
// redirects to this scheme. Keep in sync with `app.json` -> "scheme".
const MOBILE_RETURN_URL = "pokemarket://wallet/return";
const MOBILE_REFRESH_URL = "pokemarket://wallet";
...
export async function GET(request: Request) {
  try {
    ...
    if (!stripeAccountId) {
      ...
      const account = await stripe.accounts.create({
        ...
        business_profile: {
          ...
          support_email:
            process.env.SUPPORT_EMAIL ?? "support@pokemarket.fr",
        },
    ...
    // Mobile flow opens the URL in `expo-web-browser` and waits for the
    // deep-link scheme to fire, so we hand Stripe the deep link directly
    // instead of bouncing through a web page first.
    const accountLink = await stripe.accountLinks.create({
      account: stripeAccountId,
      type: "account_onboarding",
      return_url: isMobile ? MOBILE_RETURN_URL : `${appUrl}/wallet/return`,
      refresh_url: isMobile ? MOBILE_REFRESH_URL : `${appUrl}/wallet`,
    });
