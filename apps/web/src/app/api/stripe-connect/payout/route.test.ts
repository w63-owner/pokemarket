...
describe("payout — Fix D: clé d'idempotence unique par tentative", () => {
  ...
  it("échec du transfer Stripe → restaure le solde par ajout atomique sans écraser un crédit concurrent", async () => {
    const db = createMockDb(payoutScenario(50));
    mockClient = db.client;

    transfersCreate.mockImplementationOnce(async () => {
      // Simule une vente finalisée pendant que Stripe rejette le transfer :
      // l'ancien code faisait SET available_balance = 50 WHERE balance = 0,
      // donc cette condition ne matchait plus et les 50 EUR réservés restaient
      // perdus. Le RPC doit additionner 50 au nouveau crédit de 10.
