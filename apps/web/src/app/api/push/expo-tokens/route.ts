...
export async function POST(request: Request) {
  ...
  // already verified the caller with `getRequestUser`. The unique key is the
  // token itself, not (user, token): a shared device must transfer ownership to
  // the latest signed-in account or notifications leak across users.
