-- 00058: Harden wallet accounting and Expo push token ownership.
--
-- Fixes three critical failure modes:
--   1. Escrow release must move the exact pending amount credited at payment
--      time (total_amount - fee_amount, including shipping passthrough) before
--      marking a transaction COMPLETED.
--   2. Payout pre-transfer restore needs an atomic increment helper so a
--      concurrent wallet credit cannot be overwritten.
--   3. A physical Expo push token can only belong to one user at a time; when
--      a shared device changes accounts, ownership must move instead of
--      leaking the previous user's notifications to the new session.

-- ── Expo tokens: one token, one current owner ────────────────────────────────
WITH ranked AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
...
DELETE FROM public.expo_push_tokens t
USING ranked r
WHERE t.id = r.id
  AND r.rn > 1;
...
-- ── Wallet restore helper: atomic additive update ────────────────────────────
...
BEGIN
  IF p_amount <= 0 THEN
    RAISE EXCEPTION 'INVALID_AMOUNT: p_amount must be > 0 (got %)', p_amount
      USING ERRCODE = 'P0001';
  END IF;
...
-- ── Status guard: authenticated buyers must use escrow RPC for completion ───
...
BEGIN
  ...
  -- SHIPPED → COMPLETED: regular buyers must go through release_escrow_funds()
  -- so the wallet movement and status transition remain atomic.
  IF NEW.status = 'COMPLETED' AND OLD.status = 'SHIPPED' THEN
    release_marker := current_setting('pokemarket.release_escrow_funds', true);
...
-- ── Escrow release: wallet first, full seller net, fail closed ───────────────
...
BEGIN
  ...
  -- The payment finalizer credits pending_balance with card net plus shipping
  -- passthrough, persisted as total_amount - fee_amount.
  v_seller_net := ROUND(
  ...
  PERFORM set_config('pokemarket.release_escrow_funds', p_transaction_id::TEXT, TRUE);
