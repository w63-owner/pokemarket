...
BEGIN
  ...
  LOOP
    ...
    -- A transfer reversal can race a pending payout. The reversal journal has
    -- already consumed the corresponding connected/payout-pending balance, so
    -- a later payout event must only clear the reservation, never move the
