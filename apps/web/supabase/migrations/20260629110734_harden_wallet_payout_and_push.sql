-- Harden financial wallet transitions and mobile push-token ownership.

-- A physical device token must belong to exactly one account. If logout token
-- deletion fails and another user logs in on the same app install, keeping both
-- rows leaks private push notification content across accounts.
...
-- Restore wallet balances after pre-transfer failures with an atomic increment
-- instead of an absolute "set it back" write that can clobber concurrent sales.
...
BEGIN
  ...
  UPDATE public.wallets
     SET available_balance = ROUND(
       COALESCE(available_balance, 0::NUMERIC) + ROUND(p_amount, 2),
       2
     )
...
-- Direct updates to COMPLETED bypass the escrow RPC and strand seller funds in
-- pending_balance. Require the RPC to set a transaction-local marker before the
-- status transition can pass.
...
BEGIN
  ...
  IF NEW.status = 'COMPLETED' AND OLD.status = 'SHIPPED' THEN
    IF current_setting('pokemarket.release_escrow_funds', TRUE) IS DISTINCT FROM 'on' THEN
      RAISE EXCEPTION 'Unauthorized: COMPLETED transition must use release_escrow_funds'
        USING ERRCODE = '42501';
    END IF;
  ...
  -- transitions performed by webhooks, cron, or admin server routes.
  IF caller_id IS NULL THEN
    RETURN NEW;
  END IF;

  -- Users with the admin role are unrestricted for other status transitions.
  SELECT (role = 'admin') INTO caller_is_admin
  FROM public.profiles
  WHERE id = caller_id;
...
BEGIN
  ...
  -- Payment finalization credits pending_balance with card net plus shipping.
  -- fee_amount is computed only on the card price, so subtracting shipping here
  -- strands the seller's shipping reimbursement forever.
  v_seller_net := ROUND(
  ...
  -- Move wallet funds before marking COMPLETED. If the pending wallet credit is
  -- missing, fail the RPC so the order remains reconcilable instead of giving
  -- the buyer completion while silently underpaying the seller.
  UPDATE public.wallets
