-- Critical wallet and push-token guards.
--
-- 1. Expo push tokens represent a physical app install, so a token must have a
--    single current owner. Keep the newest row for any duplicates, then enforce
--    uniqueness by token to prevent cross-account notification delivery.
-- 2. Escrow release must only mark an order COMPLETED after the seller wallet
--    pending -> available move succeeds. The seller receives card net plus
--    shipping, which is equivalent to total_amount - fee_amount.
...
BEGIN
  ...
  -- Seller receives card net plus shipping. Because fee_amount is computed as
  -- display_price - card_net, this is total_amount - fee_amount.
  v_seller_net := ROUND(
