-- 00058: Harden escrow release accounting
--
-- Payment finalization credits the seller's pending balance with:
--   card net proceeds + shipping passthrough
--
-- Since transactions.total_amount already includes shipping and fee_amount is
-- calculated only on the card display price, the amount to release is:
--   seller_net = total_amount - fee_amount
--
-- Move the wallet funds before marking the transaction COMPLETED so a missing
-- or under-credited wallet fails closed instead of silently completing an order
-- without making the seller's funds withdrawable.
