-- 00058: Harden escrow release accounting and completion path
--
-- Previous release logic moved only card proceeds from pending -> available,
-- leaving shipping stuck in pending_balance, and marked transactions COMPLETED
-- before proving the wallet movement succeeded. It also allowed buyers to
-- PATCH SHIPPED -> COMPLETED directly through RLS, bypassing escrow release.
...
BEGIN
  ...
  -- SHIPPED -> COMPLETED must happen through release_escrow_funds so the
  -- seller wallet mutation and status transition stay atomic.
  IF NEW.status = 'COMPLETED' AND OLD.status = 'SHIPPED' THEN
    IF current_setting('app.escrow_release', true) IS DISTINCT FROM '1' THEN
...
BEGIN
  ...
  -- Seller escrow credit includes card proceeds after fees plus shipping.
  -- post-payment stores that as total_amount - fee_amount.
  v_seller_net := ROUND(
  ...
  -- Move funds before completing the order. If this UPDATE does not affect a
  -- row, abort the function so the transaction remains SHIPPED and operators
  -- can reconcile the missing pending balance.
  ...
  -- Mark this statement as the authorized escrow-release completion path.
  PERFORM set_config('app.escrow_release', '1', true);
