-- 00058: Harden wallet payout accounting.
--
-- Fixes two escrow-release invariants:
--   1. Shipping is pass-through seller money. finalizePaidTransaction credits
--      pending_balance with total_amount - fee_amount, so release must move
--      that exact amount instead of subtracting shipping a second time.
--   2. A transaction must not become COMPLETED unless the seller wallet was
--      actually moved from pending_balance to available_balance.
...
BEGIN
  -- auth.uid() is NULL for service_role (webhooks, cron, admin server code).
  -- Regular JWT callers must be the buyer themselves or an admin.
  ...
  -- Seller receives card net plus shipping pass-through:
  -- total_amount = display_price + shipping_cost
  -- fee_amount   = display_price - card_net
  -- seller_net   = card_net + shipping_cost = total_amount - fee_amount
  v_seller_net := ROUND(
  ...
  -- Move wallet funds before completing the transaction. If the seller wallet
  -- is missing or has drifted below the expected pending amount, raising here
  -- keeps the order SHIPPED so it can be retried/reconciled safely.
