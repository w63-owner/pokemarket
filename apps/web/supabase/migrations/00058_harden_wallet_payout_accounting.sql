-- 00058: Harden payout/escrow wallet accounting
--
-- Fixes two critical financial invariants:
--   1. Escrow release moves exactly the seller amount that was credited at
--      payment time: total_amount - fee_amount (shipping is passed through
--      and must not be subtracted twice).
--   2. A transaction is marked COMPLETED only after the wallet move succeeds.
--      Direct authenticated SHIPPED -> COMPLETED updates are blocked so buyers
--      cannot bypass the escrow RPC and strand seller funds in pending_balance.
...
DECLARE
  ...
  escrow_rpc_mark  TEXT;
BEGIN
  caller_id := auth.uid();

  -- service_role: auth.uid() is NULL -> unrestricted (webhooks, cron, admin
  -- client). Only user-authenticated updates need the extra ownership checks.
  ...
  -- SHIPPED -> COMPLETED must go through release_escrow_funds(), which sets a
  -- transaction-local marker immediately before the status update. A direct
  -- PostgREST update by the buyer would skip the wallet movement entirely.
  IF NEW.status = 'COMPLETED' AND OLD.status = 'SHIPPED' THEN
    escrow_rpc_mark := current_setting('app.release_escrow_funds', TRUE);
    IF escrow_rpc_mark IS DISTINCT FROM 'on' THEN
      RAISE EXCEPTION 'Unauthorized: use release_escrow_funds to complete shipped transactions'
        USING ERRCODE = '42501';
    END IF;
...
BEGIN
  -- auth.uid() is NULL for service_role (cron/webhooks/admin client) and is
  -- allowed. Regular JWT callers must be the buyer or an admin.
  ...
  -- finalizePaidTransaction credits card net + shipping. Since fee_amount is
  -- computed only on the card display price, total_amount - fee_amount matches
  -- the exact amount sitting in pending_balance.
  v_seller_net := ROUND(
  ...
  -- Let the status trigger distinguish this wallet-backed transition from a
  -- direct authenticated PostgREST update. The setting is LOCAL to this DB tx.
