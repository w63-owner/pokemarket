-- Harden escrow release accounting.
--
-- Previous release_escrow_funds implementations subtracted shipping_cost when
-- moving funds from pending_balance to available_balance. Payment finalization
-- credits sellers with card proceeds after fees plus shipping, which is
-- equivalent to total_amount - fee_amount. Releasing less left shipping funds
-- permanently stuck in pending_balance.
--
-- This replacement also fails before marking the transaction COMPLETED if the
-- wallet movement cannot be applied, preventing orders from closing while the
...
BEGIN
  -- auth.uid() is NULL for service_role callers such as cron/webhooks.
  ...
  -- total_amount includes shipping; fee_amount is only the marketplace fee.
  -- Therefore total_amount - fee_amount exactly matches the amount credited to
  -- pending_balance in finalizePaidTransaction (seller card net + shipping).
  v_seller_net := ROUND(
