-- 00058: Fix escrow release accounting and fail closed on wallet mismatch.
--
-- Payment finalization credits pending_balance with:
--   seller card net + shipping passthrough
-- which is equivalent to:
--   total_amount - fee_amount
--
-- The previous release RPC subtracted shipping_cost a second time, leaving the
-- shipping portion permanently stuck in pending_balance. It also marked the
-- transaction COMPLETED before verifying the wallet move succeeded. This
-- replacement keeps the status and wallet mutation in one transaction, but
-- refuses to complete the order unless the full credited amount is moved.
