-- Harden payout/escrow wallet accounting.
--
-- Fixes two critical financial invariants:
--   1. Escrow release moves exactly the seller amount credited at payment
--      time: total_amount - fee_amount. Shipping is passed through and must
--      not be subtracted twice.
--   2. A transaction becomes COMPLETED only after its wallet move succeeds.
--      Authenticated callers must use the escrow RPC rather than bypassing it
--      with a direct SHIPPED -> COMPLETED update.
...
BEGIN
  ...
  -- service_role callers (webhooks, cron, admin client) are unrestricted.
  ...
  -- The RPC sets this transaction-local marker immediately before updating
  -- the status. A direct PostgREST update would otherwise strand escrow.
...
BEGIN
  ...
  -- finalizePaidTransaction credits card net plus shipping. fee_amount is
  -- computed only on card value, so this is the exact pending credit.
  v_seller_net := ROUND(
  ...
  -- Move funds first. If the wallet is missing or underfunded, raising aborts
  -- the whole transaction and leaves the order SHIPPED for reconciliation.
